...
public class QuasiNewtonMethod2 implements OptimizationMethod<ValuePointTelemetry> {
  ...
  private double min = -5.0;          // parameter minimum

  @Property(name = "Parameter Maximum - Limits the Search Space")
  @Range(from = -Double.MAX_VALUE, to = Double.MAX_VALUE)
  private double max = 5.0;           // parameter maximum

  /**
   * Line search type.
   */
  @Property(name = "Line Search Type")
  private LineSearchType lineSearch = LineSearchType.BRENT_WITH;
  ...
  // TODO
   //init(function, Point.random(dimension, min, max).toArray());
  }
  
  // jeste jednou pretizit
  
  // TODO: zobecnit xValue, yValue
  public void init(ObjectiveFunction function, double[] xValue, double yValue) {
    this.function = function;
    dimension = function.getDimension();
    ...
    x = xValue;
    solution = ValuePoint.at(Point.at(x), yValue);
