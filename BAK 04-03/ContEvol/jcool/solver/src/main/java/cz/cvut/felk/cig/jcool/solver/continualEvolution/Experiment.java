...
public class Experiment {
     ...
     */
    public static void CEA_OPTIMIZER(JNIfgeneric fgeneric, int dim, double maxfunevals, Random rand) {
      // Obtain the target function value, which only use is termination 
      double targetFitnessValue = fgeneric.getFtarget();
      double bestFitnessValue;         // best fitness function value found
      int initSize = 5;//INIT_SIZE;        // initial population size
      int maxSize = 10;//MAX_SIZE;          // max population size
      int i = 0;                       // number of CEA iterations
      if (maxfunevals > 1e9 * dim) {
           maxfunevals = 1e9 * dim;
      }
      
      MyFitness myFitness = new MyFitness(fgeneric, (int)maxfunevals, dim); 
      Cea cea = new Cea(myFitness, dim, initSize, maxSize);

      while(true) {
        if (myFitness.noMoreEvaluations())  // number of fitness evaluations exhausted 
          break;
        i++;
    ...
    public static void main(String[] args) {
        ...
        params.algName = "CEA";
        params.comments = "Continual Evolution Algorithm";
        ...
        for (idx_dim = 0; idx_dim < 6; idx_dim++) {
          ...
          for (ifun = 2; ifun <= 2; ifun++) { //Noiseless testbed
                for (idx_instances = 0; idx_instances < 1; idx_instances++) {
                    ...
                    while (fgeneric.getEvaluations() < maxfunevals) {
                        ...
                        CEA_OPTIMIZER(fgeneric, dim[idx_dim],maxfunevals - fgeneric.getEvaluations(), rand);
                        //MY_OPTIMIZER(fgeneric, dim[idx_dim],maxfunevals - fgeneric.getEvaluations(), rand);
