package cz.cvut.felk.cig.jcool.benchmark.method.direct;
 ...
 */
@Component(name = "Dividing rectangle optimization")
public class DirectMethod implements OptimizationMethod<ValuePointListTelemetryColored> {
  ...
  private double max = 10.0;                     // parameter maximum
        ...
        private ArrayList<Double[]> lengthsSide; //array with all rectangle side lengths in each dimension
        ...
        private ValuePointListTelemetryColored telemetry;
  ...
  // remember Consumers interested in my telemetry
        private Consumer<? super ValuePointListTelemetryColored> consumer;

    public DirectMethod() {
        this.stopCondition = new SimpleStopCondition();
        this.stopCondition.init(minFunctionValue, MachineAccuracy.EPSILON, MachineAccuracy.SQRT_EPSILON, 20);
        this.telemetry = new ValuePointListTelemetryColored();
    }
    ...
    public void init(ObjectiveFunction function) {
        ...
        minBounds=function.getMinimum();
        if(minBounds[0]== -Double.MAX_VALUE){
            Arrays.fill(minBounds, min);
        }
        maxBounds=function.getMaximum();
        if(maxBounds[0]==Double.MAX_VALUE){
            Arrays.fill(maxBounds, max);
        }
        dimension=function.getDimension();
        resultMinimum=new double[2];
        resultMinimum[1]=0;
        ...
        minFunctionValue =0;
        Double [] cTmp =new Double [dimension];
        double [] cooToSample= new double[dimension];
        Double [] lTmp =new Double [dimension];
        Double dTmp =0.0;
        Double [] cooTmp =new Double [dimension];
        //transfor the domain into the unit hyper-cube
        for(int i=0;i<dimension;i++){
            cTmp[i]=0.5;
            lTmp[i]=0.5;
            dTmp=dTmp+cTmp[i]*cTmp[i];
            cooToSample[i]=minBounds[i]+cTmp[i]*(maxBounds[i]-minBounds[i]);
        }
        centerPoints.add(cTmp);
        lengthsSide.add(lTmp);
        dTmp=Math.sqrt(dTmp);
        diagonals.add(dTmp);
        Point startingPoint=Point.at(cooToSample);
        //sampling center point
        minFunctionValue =function.valueAt(startingPoint);
        stopCondition.setInitialValue(minFunctionValue);
        sampledPoints = sampledPoints +1;
        for(int i1=0;i1<dimension;i1++){
           cooTmp[i1]=cooToSample[i1];
        }
    ...
    public void optimize() {
        ...
        //For each potentially optimal rectangle
            ...
            update();
            telemetry = new ValuePointListTelemetryColored(coordinates);
    ...
    *conhull returns all points on the convex hull, even redundant ones.
    ...
    private double[] conhull(double[] x, double[] y) {
        ...
        while ((next(v, m) != 0) || (flag == 0)) {
            ...
            b = next(v, m);
            c = next(next(v, m), m);
            determinant = (x[a] * y[b] * 1) + (x[b] * y[c] * 1) + (x[c] * y[a] * 1) - (1 * y[b] * x[c]) - (1 * y[c] * x[a]) - (1 * y[a] * x[b]);
    ...
    *Divide the rectangle containing centerPoints.get(j) into thirds along the
    ...
    /**
    *Determine where to sample within rectangle j and how to
    divide the rectangle into subrectangles. Update minFunctionValue
    ...
    private void samplingPotentialRec(int j){
     ...
     /**
    *returns array with elements which pole[i]==cislo
     points sampled.
     * @param pole
     * @param cislo
      * @return
    ...
    *returns array with elements which pole[i]!=cislo
    ...
    /**
    *returns array with elements which pole[i]==pole1[a]
     points sampled.
     * @param pole
     * @param pole1
      * @return
    ...
    *Identify the set of all potentially optimal rectangles
    ...
    private ArrayList<Integer> identifyPotentiallyRec() {
       ...
       // s_1 now includes all rectangles i, with diagonals[i] >= diagonals(indexPotentialBestRec)
        if(diagonalsDifferent.size()-indx[0]>2){
               double a1= diagonals.get(indexPotentialBestRec),a2= diagonalsDifferent.get(diagonalsDifferent.size()-1),b1= functionValues.get(indexPotentialBestRec),b2= diagonalsMinFunc[diagonalsDifferent.size()-1];
    ...
    public void addConsumer(Consumer<? super ValuePointListTelemetryColored> consumer) {
        this.consumer = consumer;
    }

    public ValuePointListTelemetryColored getValue() {
        return telemetry;
    }
