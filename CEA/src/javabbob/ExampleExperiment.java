...
public class ExampleExperiment {
    ...
    public static void CEA_OPTIMIZER(JNIfgeneric fgeneric, int dim, double maxfunevals, Random rand) {
      ...
      //int initSize = 5;
      int initSize = Math.min(dim/2+4, (int)maxfunevals);      
      int maxSize = initSize*2;

      Fitness myFitness = new MyFitness(fgeneric, (int)maxfunevals); 
        ...
        for (idx_dim = 0; idx_dim < 6; idx_dim++) {
            ...
            for (ifun = 1; ifun <= 24; ifun++) { //Noiseless testbed
                for (idx_instances = 0; idx_instances < 1; idx_instances++) {
                    ...
                    maxfunevals = 50 * dim[idx_dim]; /* PUT APPROPRIATE MAX. FEVALS */
